"""
Chatbox widget for displaying individual messages.
"""
...
from weave.tui.models import ChatMessage, WeaveModel
...
class SelectionTextArea(TextArea):
    """TextArea with vim-style selection mode."""
...
class Chatbox(Widget, can_focus=True):
    ...
    def on_mount(self) -> None:
        role = self.message.message.get("role", "user")
        if role == "assistant":
    ...
    def render(self) -> RenderableType:
        ...
        weave_app: "Weave" = self.app  # type: ignore
        theme = weave_app.theme_object
        if theme:
